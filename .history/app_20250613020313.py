...
def export_pdf():
    ...
    try:
        ...
        elements.append(text_handler.create_paragraph(
            "Các tiêu chí được sử dụng trong phân tích:",
            styles['VNNormal']
        ))

        # Criteria weights table
        criteria_data = [
            ['Tiêu chí', 'Trọng số', 'Tỷ trọng']
        ]
...
def process_vietnamese_text(text):
    """Process Vietnamese text for PDF generation"""
    if not isinstance(text, str):
        return str(text)
...
# Add this function to handle paragraph creation
def create_vn_paragraph(text, style):
    """Create a paragraph with Vietnamese text"""
    try:
        # Process text
        processed_text = process_vietnamese_text(text)
        # Create paragraph

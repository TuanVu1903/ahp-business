...
def export_pdf():
    ...
    try:
        ...
        # Enhanced custom styles using full font family
        styles.add(ParagraphStyle(
            name='CorporateCoverHeader',
            fontName='DejaVuSerif-Bold',  # Elegant serif for main title
            fontSize=32,
            alignment=TA_CENTER,
        ...
        elements.append(create_safe_paragraph("BÁO CÁO PHÂN TÍCH", styles['CorporateCoverHeader']))
        elements.append(create_safe_paragraph("PHƯƠNG PHÁP PHÂN TÍCH THỨ BẬC (AHP)", styles['CorporateCoverSubHeader']))
        elements.append(Spacer(1, 20))
        elements.append(HRFlowable(width=450, color=ACCENT_COLOR, thickness=2))
        
        elements.append(Spacer(1, 40))
        elements.append(create_safe_paragraph(goal, styles['CorporateCoverSubHeader']))
        ...
        if sorted_alternatives:
            ...
            elements.append(create_safe_paragraph(conclusion_text, styles['CorporateBodyText']))
            elements.append(Spacer(1, 10))
            
            # Best alternative
            elements.append(create_safe_paragraph(best_alt, styles['CorporateHighlight']))
            elements.append(Spacer(1, 10))
            
            # Score details
            score_text = f"với điểm số {best_score:.4f} (chiếm {percentage:.2f}% tổng điểm)."
            elements.append(create_safe_paragraph(score_text, styles['CorporateBodyText']))

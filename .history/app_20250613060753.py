...
def get_llm_suggestions(goal, file_content=None):
    ...
    """
    # Kiểm tra nếu mục tiêu có chứa từ ngữ không phù hợp
    if contains_inappropriate_language(goal):
        return {"error": "Mục tiêu chứa từ ngữ không phù hợp. Vui lòng nhập lại."}
    ...
    try:
        ...
        if 'choices' in response_data and len(response_data['choices']) > 0:
            ...
            # Kiểm tra nếu nội dung trả về từ AI có từ ngữ không phù hợp
            if check_ai_response_inappropriate(content):
                return {"error": "Mục tiêu không phù hợp hoặc có nội dung vi phạm quy định. Vui lòng nhập mục tiêu khác."}
...
def generate_criteria_matrix_ai(goal, criteria):
    ...
    """
    try:
        # Kiểm tra từ ngữ không phù hợp trong mục tiêu
        if contains_inappropriate_language(goal):
            print(f"Phát hiện mục tiêu không phù hợp: {goal}")
...
- Ma trận phải có CR < 0.1 để đảm bảo tính nhất quán."""
        ...
        for attempt in range(max_attempts):
            ...
            if 'choices' in response_data and len(response_data['choices']) > 0:
                ...
                # Kiểm tra nếu nội dung trả về từ AI có dấu hiệu từ chối
                if check_ai_response_inappropriate(content):
                    print(f"Phát hiện nội dung không phù hợp trong phản hồi AI (lần {attempt + 1})")
                    continue
...
def generate_alternative_matrix_ai_function(goal, criterion, alternatives):
    ...
    """
    try:
        # Kiểm tra từ ngữ không phù hợp trong mục tiêu hoặc tiêu chí
        if contains_inappropriate_language(goal) or contains_inappropriate_language(criterion):
            print(f"Phát hiện mục tiêu hoặc tiêu chí không phù hợp: {goal} - {criterion}")
...
- Ma trận phải có CR < 0.1"""
        ...
        for attempt in range(max_attempts):
            ...
            if 'choices' in response_data and len(response_data['choices']) > 0:
                ...
                # Kiểm tra nếu nội dung trả về từ AI có dấu hiệu từ chối
                if check_ai_response_inappropriate(content):
                    print(f"Phát hiện nội dung không phù hợp trong phản hồi AI (lần {attempt + 1} - {criterion})")
                    continue
...
def auto_generate_complete_ahp():
    ...
    try:
        ...
        if contains_inappropriate_language(goal):
            return jsonify({"error": "Mục tiêu chứa từ ngữ không phù hợp. Vui lòng nhập lại."}), 400
...
def export_excel():
    ...
    try:
        ...
        worksheet.write(row, 1, goal, cell_format)
        row += 2
        
        # Tiêu chí và trọng số
        worksheet.write(row, 0, 'TIÊU CHÍ VÀ TRỌNG SỐ', header_format)
        worksheet.merge_range(row, 0, row, 2, 'TIÊU CHÍ VÀ TRỌNG SỐ', header_format)
        row += 1
        
        worksheet.write(row, 0, 'Tiêu chí', subheader_format)
        worksheet.write(row, 1, 'Trọng số', subheader_format)
        worksheet.write(row, 2, 'Xếp hạng', subheader_format)
        row += 1
        
        # Sắp xếp tiêu chí theo trọng số giảm dần
        sorted_criteria = sorted(range(len(criteria)), key=lambda i: criteriaWeights[i], reverse=True)
        
        for i, idx in enumerate(sorted_criteria):
            worksheet.write(row, 0, criteria[idx], cell_format)
            worksheet.write(row, 1, criteriaWeights[idx], decimal_format)
            worksheet.write(row, 2, i + 1, cell_format)
            row += 1
        
        row += 2
        
        # Ma trận so sánh các tiêu chí
        if criteriaMatrix and len(criteriaMatrix) > 0:
            worksheet.write(row, 0, 'MA TRẬN SO SÁNH CÁC TIÊU CHÍ', header_format)
            worksheet.merge_range(row, 0, row, len(criteria), 'MA TRẬN SO SÁNH CÁC TIÊU CHÍ', header_format)
            row += 1
            
            # Header row với tên tiêu chí
            for i, criterion in enumerate(criteria):
                worksheet.write(row, i + 1, criterion, subheader_format)
            row += 1
            
            # Dữ liệu ma trận
            for i, criterion_row in enumerate(criteriaMatrix):
                worksheet.write(row, 0, criteria[i], subheader_format)
                for j, value in enumerate(criterion_row):
                    worksheet.write(row, j + 1, value, decimal_format)
                row += 1
            
            row += 2
        
        # Ma trận so sánh phương án theo từng tiêu chí
        for criterion in criteria:
            if criterion in alternativeMatrices and 'matrix' in alternativeMatrices[criterion]:
                matrix = alternativeMatrices[criterion]['matrix']
                
                worksheet.write(row, 0, f'Tiêu chí: {criterion}', header_format)
                worksheet.merge_range(row, 0, row, len(alternatives), f'Tiêu chí: {criterion}', header_format)
                row += 1
                
                # Header row với tên phương án
                worksheet.write(row, 0, '', subheader_format)
                for i, alt in enumerate(alternatives):
                    worksheet.write(row, i + 1, alt, subheader_format)
                row += 1
                
                # Dữ liệu ma trận
                for i, alt_row in enumerate(matrix):
                    worksheet.write(row, 0, alternatives[i], subheader_format)
                    for j, value in enumerate(alt_row):
                        worksheet.write(row, j + 1, value, decimal_format)
                    row += 1
                
                # Trọng số
                if 'weights' in alternativeMatrices[criterion]:
                    weights = alternativeMatrices[criterion]['weights']
                    row += 1
                    worksheet.write(row, 0, 'Trọng số:', subheader_format)
                    for i, weight in enumerate(weights):
                        worksheet.write(row, i + 1, weight, decimal_format)
                
                row += 2
        
        # Phương án
        worksheet.write(row, 0, 'PHƯƠNG ÁN', header_format)
        worksheet.merge_range(row, 0, row, 2, 'PHƯƠNG ÁN', header_format)
        row += 1
        
        worksheet.write(row, 0, 'Phương án', subheader_format)
        worksheet.write(row, 1, 'Điểm số', subheader_format)
        worksheet.write(row, 2, 'Xếp hạng', subheader_format)
        row += 1
        ...
        # Kết quả cuối cùng
        worksheet.write(row, 0, 'KẾT QUẢ CUỐI CÙNG', header_format)
        worksheet.merge_range(row, 0, row, 2, 'KẾT QUẢ CUỐI CÙNG', header_format)
        row += 1
        ...
        worksheet.write(row, 2, 'Xếp hạng', subheader_format)
        row += 1
        # Đóng writer để lưu dữ liệu vào buffer
        writer.close()
    ...
    try:
        ...
        for i, alt in enumerate(sorted_alternatives):
            ...
            final_data.append([
                ...
                Paragraph(f"{score:.4f} ({percentage:.2f}%)", styles['CorporateTableCell'])
            ])
        ...
        signature_data = [
            [Paragraph("Người phê duyệt<br/>(Ký, ghi rõ họ tên)<br/><br/><br/><br/><br/><br/>..................................", styles['CorporateBodyText']),
             Paragraph("Người thực hiện phân tích<br/>(Ký, ghi rõ họ tên)<br/><br/><br/><br/><br/><br/>..................................", styles['CorporateBodyText'])]
        ]
...
# Thêm hàm kiểm tra từ ngữ không phù hợp
def contains_inappropriate_language(text):
    """
    Kiểm tra xem văn bản có chứa từ ngữ không phù hợp hay không
    """
    inappropriate_words = [
        "sex", "khiêu dâm", "porn", "xxx", "đụ", "địt", "làm tình", 
        "fuck", "bitch", "shit", "damn", "cunt", "asshole", "dick",
        "lồn", "buồi", "cặc", "cu", "chim", "dái", "đéo", "đít", "đái", 
        "đụ má", "đụ mẹ", "đm", "dmm", "vcl", "vl", "vãi", "loz", "lồz", "lòn",
        "cac", "lon", "buoi", "đụ", "địt", "đmm", "clm", "cc", "cl",
        "trốn thuế", "gian lận", "lừa đảo", "hack", "tấn công", "tội phạm", "bất hợp pháp",
        "rửa tiền", "buôn lậu", "ma túy", "vũ khí", "khủng bố", "giết người", "tự tử",
        "phân biệt chủng tộc", "phân biệt giới tính", "kỳ thị", "phỉ báng", "bôi nhọ",
        "ấu dâm", "lạm dụng", "bạo lực"
    ]
...
def check_ai_response_inappropriate(content):
    """
    Kiểm tra nếu nội dung trả về từ AI có dấu hiệu từ chối vì nội dung không phù hợp
    """
    refusal_patterns = [
        "xin lỗi", "không thể hỗ trợ", "bất hợp pháp", "không đạo đức", 
        "không thể cung cấp", "không phù hợp", "vi phạm", "không được phép",
        "từ chối", "không thể giúp", "không thể đưa ra", "không thể tạo",
        "không thể thực hiện", "không thể phục vụ", "không thể đáp ứng",
        "không thể trả lời", "không thể cung cấp", "không thể hỗ trợ",
        "không thể tham gia", "không thể tư vấn", "không thể đề xuất",
        "tôi không thể", "tôi từ chối", "tôi không được phép"
    ]

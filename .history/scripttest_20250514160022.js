...
const AHPCalculator = {
    ...
    displayCriteriaMatrix: function() {
        ...
        // Data rows
        this.data.criteriaMatrix.forEach((row, i) => {
            html += `<tr><th>${this.data.selectedCriteria[i]}</th>`;
            row.forEach(val => {
                html += `<td>${typeof val === 'number' ? this.formatValue(val) : val}</td>`;
            });
            html += "</tr>";
        });
    ...
    displayAlternativeMatrices: function() {
        ...
        this.data.selectedCriteria.forEach(criterion => {
            // Bỏ qua nếu không có ma trận cho tiêu chí này
            if (!this.data.alternativeMatrices[criterion]) return;
            
            const matrix = this.data.alternativeMatrices[criterion];
            
            // Tạo section cho tiêu chí này
            ...
            matrix.forEach((row, i) => {
                tableHtml += `<tr><th>${this.data.selectedAlternatives[i]}</th>`;
                row.forEach(val => {
                    tableHtml += `<td>${typeof val === 'number' ? this.formatValue(val) : val}</td>`;
                });
                tableHtml += "</tr>";
            });
            ...
            // Tạo element cho hiển thị trọng số
            ...
            // Tạo element cho kiểm tra độ nhất quán
    ...
    calculate: function() {
        ...
        this.data.criteriaWeights = this.calculateWeights(this.data.criteriaMatrix);
        const criteriaConsistency = this.checkConsistency(this.data.criteriaMatrix, this.data.criteriaWeights);
        
        // Hiển thị trọng số tiêu chí
        const criteriaWeightsBody = document.getElementById("criteriaWeightsBody");
        if (criteriaWeightsBody) {
            criteriaWeightsBody.innerHTML = "";
            
            this.data.selectedCriteria.forEach((criterion, i) => {
                const row = document.createElement("tr");
                row.innerHTML = `
                    <td>${criterion}</td>
                    <td>${this.data.criteriaWeights[i].toFixed(4)}</td>
                `;
        ...
        this.data.selectedCriteria.forEach(criterion => {
            ...
            const weights = this.calculateWeights(this.data.alternativeMatrices[criterion]);
            const consistency = this.checkConsistency(this.data.alternativeMatrices[criterion], weights);
            
            this.data.alternativeWeights[criterion] = weights;
            ...
            if (weightsDiv) {
                ...
                let weightsHtml = `
                    <tr>
                        ${this.data.selectedAlternatives.map((alt, i) => `
                            <th>${alt}</th>
                        `).join('')}
                    </tr>
                    <tr>
        ...
        if (alternativeWeightsHead && alternativeWeightsBody) {
            // Header row
            ...
            this.data.selectedCriteria.forEach(criterion => {
                // Bỏ qua nếu không có trọng số cho tiêu chí này
                if (!this.data.alternativeWeights[criterion]) return;
                
                const row = document.createElement("tr");
                let rowHtml = `<td>${criterion}</td>`;
                
                this.data.selectedAlternatives.forEach((alt, i) => {
                    rowHtml += `<td>${this.data.alternativeWeights[criterion][i].toFixed(4)}</td>`;
                });
        ...
        this.data.selectedAlternatives.forEach((alt, i) => {
            ...
            this.data.selectedCriteria.forEach((criterion, j) => {
                // Bỏ qua nếu không có trọng số cho tiêu chí này
                if (!this.data.alternativeWeights[criterion]) return;
                
                score += this.data.criteriaWeights[j] * this.data.alternativeWeights[criterion][i];
            });
            
            this.data.finalScores[alt] = score;
        });
    ...
    displayFinalResults: function() {
        ...
        const sortedAlternatives = Object.keys(this.data.finalScores).sort(
            (a, b) => this.data.finalScores[b] - this.data.finalScores[a]
        );
        ...
        if (tableBody) {
            ...
            sortedAlternatives.forEach((alt, index) => {
                ...
                row.innerHTML = `
                    <td>${alt}</td>
                    <td>${this.data.finalScores[alt].toFixed(4)}</td>
                    <td>${index + 1}</td>
                `;
        ...
        if (ctx) {
            ...
            window.finalChart = new Chart(ctxContext, {
                type: "bar",
                data: {
                    labels: sortedAlternatives,
                    datasets: [{
                        label: "Điểm số",
                        data: sortedAlternatives.map(alt => this.data.finalScores[alt]),
                        backgroundColor: [
                            "rgba(75, 192, 192, 0.7)",
                            "rgba(54, 162, 235, 0.7)",
                            "rgba(255, 206, 86, 0.7)"
                ...
                options: {
                    scales: {
                        y: {
                            beginAtZero: true,
                            max: Math.max(...Object.values(this.data.finalScores)) * 1.1
                        }
                    }
                }
            });
    ...
    saveSession: function() {
        const sessionData = {
            goal: document.getElementById("goalInput") ? document.getElementById("goalInput").value : "Chọn điện thoại phù hợp nhất",
            criteria: this.data.selectedCriteria,
            alternatives: this.data.selectedAlternatives,
            criteriaMatrix: this.data.criteriaMatrix,
            alternativeMatrices: this.data.alternativeMatrices
        };
        
        localStorage.setItem("ahpSession", JSON.stringify(sessionData));
        alert("Phiên làm việc đã được lưu!");
    },
    
    // Tải phiên làm việc đã lưu
    loadSession: function() {
        const savedData = localStorage.getItem("ahpSession");
        if (savedData) {
            try {
                ...
                // Cập nhật dữ liệu
                this.data.selectedCriteria = sessionData.criteria || [];
                this.data.selectedAlternatives = sessionData.alternatives || [];
                this.data.criteriaMatrix = sessionData.criteriaMatrix || [];
                this.data.alternativeMatrices = sessionData.alternativeMatrices || {};
                ...
                alert("Đã tải phiên làm việc thành công!");
            } catch (error) {
                console.error("Error loading session:", error);
                alert("Có lỗi khi tải phiên làm việc: " + error.message);
            }
        } else {
            alert("Không tìm thấy phiên làm việc đã lưu!");
        }
    }
};
...
// Khởi tạo AHPCalculator khi trang đã tải xong

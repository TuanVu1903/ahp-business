...
def export_pdf():
    ...
    try:
        ...
        def create_paragraph_style(name, font_name='DejaVuSans', font_size=11, alignment=TA_JUSTIFY, text_color=None, space_before=0, space_after=0):
            """Helper function to create paragraph styles with proper encoding"""
        ...
        def create_text_paragraph(text, style_name='Normal', bold=False, italic=False):
            """Helper function to create paragraphs with proper text handling"""
            try:
                # Clean the text
                text = text.strip()
                
                # Apply bold/italic formatting without HTML
                if bold and italic:
                    style_name = style_name + 'BoldItalic'
                elif bold:
                    style_name = style_name + 'Bold'
                elif italic:
                    style_name = style_name + 'Italic'
                
                # Create the paragraph with proper encoding
        ...
        # Setup fonts with full DejaVu family
        from reportlab.pdfbase import pdfmetrics
        from reportlab.pdfbase.ttfonts import TTFont
        import os

        # Register complete DejaVu font family
        ...
        pdfmetrics.registerFont(TTFont('DejaVuSans-ExtraLight', os.path.join(font_path, 'DejaVuSans-ExtraLight.ttf')))
        
        # Serif family for elegant headings
        ...
        pdfmetrics.registerFont(TTFont('DejaVuSerif-Italic', os.path.join(font_path, 'DejaVuSerif-Italic.ttf')))
        pdfmetrics.registerFont(TTFont('DejaVuSerif-BoldItalic', os.path.join(font_path, 'DejaVuSerif-BoldItalic.ttf')))
        
        # Mono family for technical content
        pdfmetrics.registerFont(TTFont('DejaVuSansMono', os.path.join(font_path, 'DejaVuSansMono.ttf')))
        pdfmetrics.registerFont(TTFont('DejaVuSansMono-Bold', os.path.join(font_path, 'DejaVuSansMono-Bold.ttf')))
        pdfmetrics.registerFont(TTFont('DejaVuSansMono-Oblique', os.path.join(font_path, 'DejaVuSansMono-Oblique.ttf')))

        # Enhanced corporate colors
        PRIMARY_COLOR = colors.HexColor('#1e3a8a')  # Darker blue for main headers
        SECONDARY_COLOR = colors.HexColor('#1d4ed8')  # Bright blue for subheaders
        ACCENT_COLOR = colors.HexColor('#047857')  # Emerald green for highlights
        LIGHT_COLOR = colors.HexColor('#f8fafc')  # Light background
        TEXT_COLOR = colors.HexColor('#1e293b')  # Main text color
        SUBTEXT_COLOR = colors.HexColor('#475569')  # Secondary text
        SUCCESS_COLOR = colors.HexColor('#15803d')  # Success indicators
        WARNING_COLOR = colors.HexColor('#b45309')  # Warning indicators
        GRID_COLOR = colors.HexColor('#e2e8f0')  # Table grids
        HIGHLIGHT_COLOR = colors.HexColor('#dbeafe')  # Highlight backgrounds
        ...
        # Add custom styles for Vietnamese text
        styles.add(create_paragraph_style(
            'TitleVN',
        ...
        styles.add(ParagraphStyle(
            ...
            fontName='DejaVuSerif',  # Elegant serif for subtitle
            fontSize=20,
            alignment=TA_CENTER,
        ...
        styles.add(ParagraphStyle(
            ...
            fontName='DejaVuSans-Bold',  # Clean sans-serif for section headers
            fontSize=18,
            alignment=TA_LEFT,
        ...
        styles.add(ParagraphStyle(
            ...
            fontName='DejaVuSans',  # Clean sans-serif for body text
            fontSize=11,
            alignment=TA_JUSTIFY,
            textColor=TEXT_COLOR,
            leading=16,
            ...
            leftIndent=10,
            firstLineIndent=20,
            hyphenationLang='vi_VN',  # Vietnamese hyphenation
            encoding='utf-8'
        ))
        ...
        styles.add(ParagraphStyle(
            name='CorporateNote',
            fontName='DejaVuSans-Oblique',  # Italic for notes
            fontSize=9,
            alignment=TA_LEFT,
        ...
        # Title using new styles
        elements.append(Paragraph("BÁO CÁO PHÂN TÍCH", styles['TitleVN']))
        elements.append(Paragraph("PHƯƠNG PHÁP PHÂN TÍCH THỨ BẬC (AHP)", styles['HeaderVN']))
        elements.append(Spacer(1, 20))
        elements.append(HRFlowable(width=450, color=ACCENT_COLOR, thickness=2))
        
        elements.append(Spacer(1, 40))
        elements.append(Paragraph(goal, styles['HeaderVN']))
        ...
        Báo cáo này trình bày kết quả phân tích quyết định đa tiêu chí sử dụng phương pháp Phân tích thứ bậc (AHP) 
        cho mục tiêu: <b>{goal}</b>. 
        </para>
        <br/>
        <para alignment="justify">
        Phân tích được thực hiện dựa trên <b>{len(criteria)} tiêu chí</b> và <b>{len(alternatives)} phương án</b> thay thế. 
        Kết quả phân tích sẽ cung cấp cơ sở khoa học để đưa ra quyết định tối ưu dựa trên các tiêu chí đã xác định.
        </para>
        """
        elements.append(Paragraph(overview_text, styles['CorporateBodyText']))
        ...
        <br/>
        <bullet>•</bullet> Phân rã vấn đề phức tạp thành các thành phần đơn giản hơn
        <br/>
        <bullet>•</bullet> So sánh cặp để xác định mức độ quan trọng tương đối
        <br/>
        <bullet>•</bullet> Tổng hợp các đánh giá để xác định ưu tiên tổng thể
        <br/><br/>
        <para alignment="justify">
        ...
        if sorted_alternatives:
            ...
            # Main conclusion
            conclusion_text = f"Dựa trên kết quả phân tích AHP với {len(criteria)} tiêu chí và {len(alternatives)} phương án, phương án được đánh giá là phù hợp nhất là:"
            elements.append(Paragraph(conclusion_text, styles['BodyVN']))
            elements.append(Spacer(1, 10))
            
            # Best alternative
            elements.append(Paragraph(best_alt, styles['HighlightVN']))
            elements.append(Spacer(1, 10))
            
            # Score details
            score_text = f"với điểm số {best_score:.4f} (chiếm {percentage:.2f}% tổng điểm)."
            elements.append(Paragraph(score_text, styles['BodyVN']))
            ...
            if len(sorted_alternatives) > 1:
                ...
                comparison_text = f"So với phương án xếp hạng thứ hai ({second_alt}), phương án được chọn có điểm số cao hơn {diff_percent:.2f}%. Sự chênh lệch này cho thấy tính vượt trội của phương án được chọn."
                elements.append(create_safe_paragraph(comparison_text, styles['CorporateBodyText']))
        ...
        <br/>
        <bullet>•</bullet> Ưu tiên triển khai phương án <b>{sorted_alternatives[0]}</b>
        <br/>
        <bullet>•</bullet> Xây dựng kế hoạch triển khai chi tiết với các mốc thời gian cụ thể
        <br/>
        <bullet>•</bullet> Phân công trách nhiệm và nguồn lực rõ ràng
        <br/><br/>
        <b>2. Giám sát và đánh giá:</b>
        <br/>
        <bullet>•</bullet> Thiết lập hệ thống theo dõi và đánh giá định kỳ
        <br/>
        <bullet>•</bullet> Xác định các chỉ số KPI để đo lường hiệu quả
        <br/>
        <bullet>•</bullet> Tổ chức họp đánh giá tiến độ hàng tháng/quý
        <br/><br/>
        <b>3. Quản trị rủi ro:</b>
        <br/>
        <bullet>•</bullet> Xây dựng kế hoạch dự phòng cho các rủi ro tiềm ẩn
        <br/>
        <bullet>•</bullet> Chuẩn bị các phương án thay thế khi cần thiết
        <br/>
        <bullet>•</bullet> Thường xuyên cập nhật và điều chỉnh kế hoạch
        """
        elements.append(Paragraph(recommendations, styles['CorporateBodyText']))
...
def create_safe_paragraph(text, style):
    """Create a paragraph with safe text handling for Vietnamese characters"""
    try:
        # Remove HTML tags if present
        text = text.replace('<para>', '').replace('</para>', '')
        text = text.replace('<br/>', '\n')
        
        # Create paragraph with basic styling
        return Paragraph(text, style)
    except Exception as e:
        print(f"Error creating paragraph: {str(e)}")
        # Fallback to basic text
        return Paragraph(text.encode('utf-8').decode('utf-8'), style)

class VietnameseTextHandler:
    @staticmethod
    def normalize_text(text):
        """Normalize Vietnamese text for PDF generation"""
    ...
    def create_paragraph(text, style):
        """Create a paragraph with proper Vietnamese text handling"""
        try:
            # Clean and normalize text
            text = VietnameseTextHandler.normalize_text(text)
            # Create paragraph
            return Paragraph(text, style)
        except Exception as e:
            print(f"Error creating paragraph: {str(e)}")
            # Fallback to basic ASCII text
            return Paragraph(text.encode('ascii', 'ignore').decode(), style)

def create_vietnamese_styles(base_styles):
    """Create styles optimized for Vietnamese text"""
    styles = base_styles
...
# In the export_pdf function, after creating the document:
        # Initialize text handler and styles
        text_handler = VietnameseTextHandler()
        styles = create_vietnamese_styles(getSampleStyleSheet())
        elements = []
        ...
        # Title
        elements.append(text_handler.create_paragraph(
            "BÁO CÁO PHÂN TÍCH",
            styles['VNTitle']
        ))
        ...
        # Introduction
        intro_text = "Báo cáo này trình bày kết quả phân tích quyết định đa tiêu chí sử dụng phương pháp Phân tích thứ bậc (AHP) cho mục tiêu:"
        elements.append(text_handler.create_paragraph(
            intro_text,
            styles['VNNormal']
        ))
        ...
        # Analysis details
        analysis_text = f"Phân tích được thực hiện dựa trên {len(criteria)} tiêu chí và {len(alternatives)} phương án thay thế. "
        analysis_text += "Kết quả phân tích sẽ cung cấp cơ sở khoa học để đưa ra quyết định tối ưu dựa trên các tiêu chí đã xác định."
        elements.append(text_handler.create_paragraph(
            analysis_text,
            styles['VNNormal']
        ))

        # ... Update the rest of the code to use text_handler.create_paragraph ...

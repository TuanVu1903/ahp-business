...
def import_excel():
    """Import Excel file to visualize AHP process"""
    try:
        ...
        # Kiểm tra sheet "Input Data" tồn tại
        if 'Input Data' not in workbook.sheetnames:
            return jsonify({"error": "File Excel không đúng định dạng (không có sheet Input Data)"}), 400
        
        # Lấy dữ liệu tiêu chí và trọng số từ sheet "Input Data"
        input_sheet = workbook['Input Data']
        
        # Lấy tiêu đề mục tiêu nếu có (ở sheet Results)
        goal = ""
        if 'Results' in workbook.sheetnames:
            results_sheet = workbook['Results']
            if results_sheet.cell(1, 1).value:
                goal_text = results_sheet.cell(1, 1).value
                if ":" in goal_text:
                    goal = goal_text.split(":", 1)[1].strip()
        
        # Lấy danh sách tiêu chí và trọng số
        criteria = []
        criteria_weights = []
        
        row = 2  # Bắt đầu từ dòng 2 (sau header)
        while True:
            criterion_name = input_sheet.cell(row, 1).value
            criterion_weight = input_sheet.cell(row, 2).value
            ...
            # Nếu cell có công thức thì lấy giá trị từ công thức
            if isinstance(criterion_weight, str) and criterion_weight.startswith("="):
                # Lấy giá trị trực tiếp từ cell
                try:
                    criterion_weight = float(criterion_weight.replace("=", "").strip())
                except:
                    criterion_weight = 0
            ...
            criteria_weights.append(criterion_weight)
            row += 1
            
        # Lấy danh sách phương án và trọng số từ sheet "Results"
        alternatives = []
        final_scores = {}
        
        if 'Results' in workbook.sheetnames:
            results_sheet = workbook['Results']
            row = 4  # Bắt đầu từ dòng 4 (sau header)
            while True:
                alt_name = results_sheet.cell(row, 2).value
                alt_score = results_sheet.cell(row, 3).value
                ...
                # Nếu cell có công thức thì lấy giá trị từ công thức
                if isinstance(alt_score, str) and alt_score.startswith("="):
                    try:
                        alt_score = float(alt_score.replace("=", "").strip())
                    except:
                        alt_score = 0
                ...
                final_scores[alt_name] = alt_score
                row += 1
        
        # Lấy ma trận so sánh tiêu chí từ sheet "Calculations"
        criteria_matrix = np.ones((len(criteria), len(criteria)))
        alternative_matrices = {}
        
        # Lấy ma trận phương án theo từng tiêu chí từ các sheet "Criterion-X"
        for i, criterion in enumerate(criteria):
            sheet_name = f"Criterion-{i+1}"
            if sheet_name in workbook.sheetnames:
                criterion_sheet = workbook[sheet_name]
                
                # Tạo ma trận cho tiêu chí
                alt_matrix = np.ones((len(alternatives), len(alternatives)))
                alt_weights = {}
                
                # Lấy trọng số phương án
                for j, alt in enumerate(alternatives):
                    alt_weight = criterion_sheet.cell(j+3, 2).value  # +3 vì có 2 dòng header
                    if isinstance(alt_weight, str) and alt_weight.startswith("="):
                        try:
                            alt_weight = float(alt_weight.replace("=", "").strip())
                        except:
                            alt_weight = 0
                    ...
                    alt_weights[alt] = alt_weight
                
                # Tạo ma trận so sánh phương án
                # Sử dụng công thức a_ij = w_i / w_j
                for i1, alt1 in enumerate(alternatives):
                    for i2, alt2 in enumerate(alternatives):
                        if i1 == i2:
                            alt_matrix[i1][i2] = 1
                        else:
                            if alt_weights[alt2] != 0:
                                alt_matrix[i1][i2] = alt_weights[alt1] / alt_weights[alt2]
                            else:
                                alt_matrix[i1][i2] = 1
                
                # Tính CR cho ma trận phương án
                try:
                    alt_result = calculate_ahp(alt_matrix)
                    cr = alt_result["cr"]
                except:
                    cr = 0
                
                # Lưu ma trận và trọng số
                alternative_matrices[criterion] = {
                    "matrix": alt_matrix.tolist(),
                    "alternatives": alt_weights,
                    "weights": list(alt_weights.values()),
                    "cr": cr
                }
        
        # Tạo ma trận so sánh tiêu chí dựa trên trọng số
        for i in range(len(criteria)):
            for j in range(len(criteria)):
                if i == j:
                    criteria_matrix[i][j] = 1
                else:
                    if criteria_weights[j] != 0:
                        criteria_matrix[i][j] = criteria_weights[i] / criteria_weights[j]
                    else:
                        criteria_matrix[i][j] = 1
        
        # Chuyển ma trận tiêu chí sang dạng string (1, 3, 5, 7, 9, 1/3, 1/5, 1/7, 1/9)
        criteria_matrix_str = []
        for i in range(len(criteria)):
            row_str = []
            for j in range(len(criteria)):
                if criteria_matrix[i][j] == 1:
                    row_str.append("1")
                elif criteria_matrix[i][j] < 1:
                    # Làm tròn thành 1/3, 1/5, 1/7, 1/9
                    inv = 1 / criteria_matrix[i][j]
                    if inv < 2:
                        row_str.append("1")
                    elif inv < 4:
                        row_str.append("1/3")
                    ...
                    # Làm tròn thành 3, 5, 7, 9
                    if criteria_matrix[i][j] < 2:
                        row_str.append("1")
                    elif criteria_matrix[i][j] < 4:
                        row_str.append("3")
                    elif criteria_matrix[i][j] < 6:
                        row_str.append("5")
                    elif criteria_matrix[i][j] < 8:
                        row_str.append("7")
                    else:
                        row_str.append("9")
            criteria_matrix_str.append(row_str)
        
        # Tính CR cho ma trận tiêu chí
        criteria_result = calculate_ahp(criteria_matrix)
        
        # Chuyển các ma trận phương án sang dạng string
        alt_matrices_str = {}
        for criterion, data in alternative_matrices.items():
            ...
            alt_matrices_str[criterion] = matrix_str
        ...
        result = {
            ...
            "criteriaMatrix": criteria_matrix_str,
            "criteriaResult": criteria_result,
            "alternativeMatrices": alt_matrices_str,
            "alternativeResults": alternative_matrices,

...
def export_pdf():
    ...
    try:
        ...
        elements.append(Paragraph("Trọng số các tiêu chí", styles['Heading2Center']))
        ...
        elements.append(Paragraph("Trọng số các phương án theo từng tiêu chí", styles['Heading2Center']))
        
        for i, criterion in enumerate(criteria):
            elements.append(Paragraph(f"Tiêu chí: {criterion}", styles['Heading3']))
            
            # Get scores for this criterion
            alt_data = [['Phương án', 'Trọng số', 'Tỷ lệ']]
            
            for alt in alternatives:
                score = 0
                if alt in alternativeScores and criterion in alternativeScores[alt]:
                    score = alternativeScores[alt][criterion]
                alt_data.append([
                    alt,
                    f"{score:.4f}",
                    f"{score*100:.2f}%"
            ...
            # Create table for this criterion
            alt_table = Table(alt_data, colWidths=[3*inch, 1.2*inch, 1.2*inch])
        ...
        elements.append(Paragraph("Kết quả phân tích cuối cùng", styles['Heading2Center']))
        ...
        final_data = [['Xếp hạng', 'Phương án', 'Điểm số', 'Tỷ lệ']]
        ...
        if sorted_alternatives:
            ...
            conclusion_text = f"Kết luận: Phương án <b>{best_alt}</b> là lựa chọn tốt nhất với điểm số {best_score:.4f} ({percentage:.2f}%)."
            ...
            if len(sorted_alternatives) > 1:
                ...
                conclusion_text += f" Phương án này vượt trội hơn phương án {second_alt} khoảng {diff_percent:.2f}%."
        ...
        date_text = f"Báo cáo được tạo vào: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"

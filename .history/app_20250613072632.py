...
def init_mongodb():
    try:
        ...
        if not mongo_uri:
            logger.warning("MONGODB_URI not set, logging to MongoDB will be disabled")
            return None
...
def log_to_mongodb(data):
    """Log dữ liệu vào MongoDB"""
    try:
        if not mongodb_client:
            logger.warning("MongoDB logging skipped - no connection")
            return
            
        db = mongodb_client['ahp_logs']
        collection = db['logs']

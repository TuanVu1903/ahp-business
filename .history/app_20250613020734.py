...
def export_pdf():
    ...
    try:
        data = request.json
        goal = process_vietnamese_text(data.get('goal', 'AHP Analysis'))
        criteria = [process_vietnamese_text(c) for c in data.get('criteria', [])]
        alternatives = [process_vietnamese_text(a) for a in data.get('alternatives', [])]
        criteriaWeights = data.get('criteriaWeights', [])
        alternativeScores = data.get('alternativeScores', {})
        finalScores = data.get('finalScores', {})
        ...
        # Add Vietnamese-specific styles

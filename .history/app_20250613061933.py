...
def export_excel():
    ...
    try:
        ...
        print("Incoming request data for export_excel:", request.data)
        ...
        # 5. Điểm cuối cùng
        worksheet.merge_range(row, 0, row, 3, 'ĐIỂM CUỐI CÙNG', section_format)
        row += 1
        
        worksheet.write(row, 0, 'Phương án', header_format)
        worksheet.write(row, 1, 'Điểm', header_format)
        worksheet.write(row, 2, 'Phần trăm', header_format)
        row += 1
        ...
        for i, alternative in enumerate(alternatives):
            worksheet.write(row, 0, alternative)
            score = finalScores.get(alternative, 0)
            worksheet.write(row, 1, score, decimal_format)
            worksheet.write(row, 2, score, percent_format)
            row += 1
        
        # Đóng writer và trả về file Excel
        writer.close()
        output.seek(0)
        
        return send_file(output, as_attachment=True, download_name=f'{safe_filename}.xlsx', mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    except Exception as e:

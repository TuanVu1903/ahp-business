...
def handle_vietnamese_text(text):
    """Handle Vietnamese text for PDF generation"""
    try:
        ...
        print(f"Error processing Vietnamese text: {str(e)}")
        return str(text)

def create_vn_paragraph(text, style):
    """Create a paragraph with Vietnamese text handling"""
    try:
        # Process text
        text = handle_vietnamese_text(text)
        # Create paragraph
...
def export_pdf():
    ...
    try:
        ...
        # Process input data
        goal = handle_vietnamese_text(data.get('goal', 'AHP Analysis'))
        criteria = [handle_vietnamese_text(c) for c in data.get('criteria', [])]
        alternatives = [handle_vietnamese_text(a) for a in data.get('alternatives', [])]
        criteriaWeights = data.get('criteriaWeights', [])
        alternativeScores = data.get('alternativeScores', {})
        finalScores = data.get('finalScores', {})
        ...
        styles.add(ParagraphStyle(
            ...
            elements.append(create_vn_paragraph("BÁO CÁO PHÂN TÍCH", styles['VNTitle']))
            elements.append(create_vn_paragraph("PHƯƠNG PHÁP PHÂN TÍCH THỨ BẬC (AHP)", styles['VNHeader']))
            elements.append(Spacer(1, 20))
            elements.append(create_vn_paragraph(goal, styles['VNHeader']))
            elements.append(Spacer(1, 20))
            ...
            elements.append(create_vn_paragraph(intro_text, styles['VNText']))
            ...
            elements.append(create_vn_paragraph(analysis_text, styles['VNText']))
            elements.append(Spacer(1, 20))

            # Criteria section
            elements.append(create_vn_paragraph("1. TIÊU CHÍ ĐÁNH GIÁ", styles['VNHeader']))
            elements.append(create_vn_paragraph("Các tiêu chí được sử dụng trong phân tích:", styles['VNText']))
            elements.append(Spacer(1, 10))

            # Criteria table
            criteria_data = [["Tiêu chí", "Trọng số", "Tỷ trọng"]]
        for i, criterion in enumerate(criteria):
            criteria_data.append([
                    criterion,
                    f"{criteriaWeights[i]:.4f}",
                    f"{criteriaWeights[i]*100:.2f}%"
            ])

            table = Table(criteria_data, colWidths=[250, 100, 100])
            ...
            # Results section
            elements.append(create_vn_paragraph("2. KẾT QUẢ PHÂN TÍCH", styles['VNHeader']))
            elements.append(Spacer(1, 10))

            # Sort alternatives by score
            sorted_alternatives = sorted(alternatives, key=lambda x: finalScores.get(x, 0), reverse=True)
        ...
        if sorted_alternatives:
            ...
            percentage = (best_score / total_score) * 100 if total_score > 0 else 0
                ...
                elements.append(create_vn_paragraph(conclusion_text, styles['VNText']))
                elements.append(Spacer(1, 10))

                elements.append(create_vn_paragraph(best_alt, styles['VNHeader']))
                elements.append(Spacer(1, 10))

                score_text = f"với điểm số {best_score:.4f} (chiếm {percentage:.2f}% tổng điểm)."
                elements.append(create_vn_paragraph(score_text, styles['VNText']))
            ...
            if len(sorted_alternatives) > 1:
                ...
                diff_percent = ((best_score - second_score) / best_score) * 100 if best_score > 0 else 0
                    ...
                    comparison_text = f"So với phương án xếp hạng thứ hai ({second_alt}), phương án được chọn có điểm số cao hơn {diff_percent:.2f}%. "
                    comparison_text += "Sự chênh lệch này cho thấy tính vượt trội của phương án được chọn."
                    elements.append(Spacer(1, 10))
                    elements.append(create_vn_paragraph(comparison_text, styles['VNText']))

            # Alternative scores table
            elements.append(Spacer(1, 20))
            elements.append(create_vn_paragraph("Bảng xếp hạng các phương án:", styles['VNText']))
            elements.append(Spacer(1, 10))

            alt_data = [["Phương án", "Điểm số", "Tỷ trọng"]]
            ...
            alt_table = Table(alt_data, colWidths=[250, 100, 100])

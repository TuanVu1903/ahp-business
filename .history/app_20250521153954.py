...
def export_pdf():
    """Export AHP analysis results to PDF file with proper Vietnamese support"""
    try:
        data = request.json
        goal = data.get('goal', 'AHP Analysis')
        criteria = data.get('criteria', [])
        ...
        # Set up fonts for Vietnamese
        from reportlab.pdfbase import pdfmetrics
        from reportlab.pdfbase.ttfonts import TTFont
        
        # Register DejaVu Sans font for Vietnamese support
        # Try to find system fonts or use a fallback
        try:
            pdfmetrics.registerFont(TTFont('DejaVu', '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'))
        except:
            # Fallback to Helvetica (might not display Vietnamese correctly)
            print("DejaVu Sans font not found, defaulting to Helvetica")
        ...
        # Add custom styles with Vietnamese-compatible font
        ...
        # Title - simplify text to avoid encoding issues
        title_text = f"Phan tich AHP: {goal}"
        elements.append(Paragraph(title_text, styles['CustomTitle']))
        elements.append(Spacer(1, 0.2*inch))
        
        # Criteria Weights Section - simplify text
        elements.append(Paragraph("Trong so cac tieu chi", styles['Heading2Center']))
        criteria_data = [['Tieu chi', 'Trong so', 'Ty le']]
        for i, criterion in enumerate(criteria):
        ...
        criteria_table = Table(criteria_data, colWidths=[3*inch, 1.2*inch, 1.2*inch])
        ...
        # Final Scores Section - simplify text
        elements.append(Paragraph("Ket qua phan tich cuoi cung", styles['Heading2Center']))
        ...
        final_data = [['Xep hang', 'Phuong an', 'Diem so', 'Ty le']]
        ...
        final_table = Table(final_data, colWidths=[0.8*inch, 3*inch, 1.2*inch, 1.2*inch])
        final_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 0), (-1, 0), 11),
            ('BOTTOMPADDING', (0, 0), (-1, 0), 8),
            ('BACKGROUND', (0, 1), (-1, 1), colors.lightgreen),  # Highlight first row (best option)
            ('GRID', (0, 0), (-1, -1), 1, colors.black),
        ]))
        ...
        if sorted_alternatives:
            ...
            conclusion_text = f"Ket luan: Phuong an {best_alt} la lua chon tot nhat voi diem so {best_score:.4f} ({percentage:.2f}%)."
            ...
            if len(sorted_alternatives) > 1:
                ...
                conclusion_text += f" Phuong an nay vuot troi hon phuong an {second_alt} khoang {diff_percent:.2f}%."
            
            elements.append(Paragraph(conclusion_text, styles['Normal-VI']))
        
        # Footer with date
        elements.append(Spacer(1, 0.5*inch))
        date_text = f"Bao cao duoc tao vao: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
        elements.append(Paragraph(date_text, styles['Normal-VI']))

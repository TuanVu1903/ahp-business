...
def export_excel():
    ...
    try:
        ...
        # Format cho các ô dữ liệu bình thường
        ...
        # Format cho ô dữ liệu có màu nền xen kẽ
        ...
        # Format cho ô dữ liệu của phương án tốt nhất
        best_option_format = workbook.add_format({
            'border': 1,
            'align': 'left',
            'valign': 'vcenter',
            'bg_color': '#dcfce7',
        ...
        best_option_number_format = workbook.add_format({
            'border': 1,
            'align': 'right',
            'valign': 'vcenter',
            'bg_color': '#dcfce7',
        ...
        best_option_percent_format = workbook.add_format({
            'border': 1,
            'align': 'right',
            'valign': 'vcenter',
            'bg_color': '#dcfce7',
        ...
        overview_sheet = workbook.add_worksheet('Tổng quan')
        
        # Thiết lập độ rộng cột cho worksheet tổng quan
        overview_sheet.set_column('A:A', 20)
        overview_sheet.set_column('B:Z', 15)
        
        # Thiết lập chiều cao hàng tiêu đề
        overview_sheet.set_row(0, 30)
        overview_sheet.set_row(1, 25)
        
        # 1. Thông tin chung
        row = 0
        overview_sheet.merge_range(row, 0, row, 5, 'BÁO CÁO PHÂN TÍCH AHP', title_format)
        row += 1
        
        # Mục tiêu
        overview_sheet.merge_range(row, 0, row, 5, goal, subtitle_format)
        row += 2
        
        # Thông tin cơ bản
        overview_sheet.merge_range(row, 0, row, 5, 'THÔNG TIN PHÂN TÍCH', section_format)
        row += 1
        
        overview_sheet.write(row, 0, 'Mục tiêu:', cell_format)
        overview_sheet.merge_range(row, 1, row, 5, goal, cell_format)
        row += 1
        
        overview_sheet.write(row, 0, 'Số tiêu chí:', cell_format)
        overview_sheet.write(row, 1, len(criteria), cell_center_format)
        row += 1
        
        overview_sheet.write(row, 0, 'Số phương án:', cell_format)
        overview_sheet.write(row, 1, len(alternatives), cell_center_format)
        row += 1
        
        overview_sheet.write(row, 0, 'Ngày tạo báo cáo:', cell_format)
        from datetime import datetime
        overview_sheet.write(row, 1, datetime.now().strftime('%d/%m/%Y %H:%M:%S'), cell_format)
        row += 2
        
        # 2. Kết quả cuối cùng
        overview_sheet.merge_range(row, 0, row, 5, 'KẾT QUẢ CUỐI CÙNG', section_format)
        row += 1
        ...
        # Tạo bảng kết quả
        overview_sheet.write(row, 0, 'Xếp hạng', header_format)
        overview_sheet.write(row, 1, 'Phương án', header_format)
        overview_sheet.write(row, 2, 'Điểm số', header_format)
        overview_sheet.write(row, 3, 'Tỷ lệ', header_format)
        row += 1
        ...
        for i, alt in enumerate(sorted_alternatives):
            ...
            percentage = score / total_score if total_score > 0 else 0
            
            # Sử dụng định dạng đặc biệt cho phương án tốt nhất
            if i == 0:
                overview_sheet.write(row, 0, i + 1, cell_center_format)
                overview_sheet.write(row, 1, alt, best_option_format)
                overview_sheet.write(row, 2, score, best_option_number_format)
                overview_sheet.write(row, 3, percentage, best_option_percent_format)
            else:
                # Sử dụng màu nền xen kẽ cho các hàng
                if i % 2 == 0:
                    overview_sheet.write(row, 0, i + 1, cell_center_format)
                    overview_sheet.write(row, 1, alt, cell_format)
                    overview_sheet.write(row, 2, score, decimal_format)
                    overview_sheet.write(row, 3, percentage, percent_format)
                else:
                    overview_sheet.write(row, 0, i + 1, cell_center_format)
                    overview_sheet.write(row, 1, alt, alt_row_format)
                    overview_sheet.write(row, 2, score, decimal_format)
                    overview_sheet.write(row, 3, percentage, percent_format)
            row += 1
        ...
        # Tạo biểu đồ kết quả
        chart_row = row + 2
        overview_sheet.merge_range(chart_row, 0, chart_row, 5, 'BIỂU ĐỒ ĐIỂM SỐ PHƯƠNG ÁN', section_format)
        chart_row += 1
        ...
        column_chart.add_series({
            'name': 'Điểm số',
            'categories': ['Tổng quan', start_row, 1, start_row + len(sorted_alternatives) - 1, 1],
            'values': ['Tổng quan', start_row, 2, start_row + len(sorted_alternatives) - 1, 2],
            'fill': {'color': '#1a56db'},
            'data_labels': {'value': True, 'num_format': '0.0000'}
        })
        
        # Thiết lập tiêu đề và các thuộc tính khác
        column_chart.set_title({'name': 'Điểm số các phương án'})
        column_chart.set_x_axis({'name': 'Phương án'})
        column_chart.set_y_axis({'name': 'Điểm số'})
        column_chart.set_style(10)
        
        # Chèn biểu đồ vào worksheet
        overview_sheet.insert_chart(chart_row, 0, column_chart, {'x_scale': 1.5, 'y_scale': 1})
        chart_row += 18  # Để trống không gian cho biểu đồ
        ...
        pie_chart.add_series({
            ...
            'categories': ['Tổng quan', start_row, 1, start_row + len(sorted_alternatives) - 1, 1],
            'values': ['Tổng quan', start_row, 3, start_row + len(sorted_alternatives) - 1, 3],
            'data_labels': {'percentage': True, 'position': 'outside_end'}
        })
        
        # Thiết lập tiêu đề và các thuộc tính khác
        pie_chart.set_title({'name': 'Tỷ lệ phần trăm các phương án'})
        pie_chart.set_style(10)
        
        # Chèn biểu đồ vào worksheet
        overview_sheet.insert_chart(chart_row, 0, pie_chart, {'x_scale': 1.5, 'y_scale': 1})
        
        # Tạo worksheet chi tiết để chứa tất cả dữ liệu
        worksheet = workbook.add_worksheet('Chi tiết')
        ...
        worksheet.merge_range(row, 1, row, 3, goal, cell_format)
        row += 2
        ...
        for i, criterion in enumerate(criteria):
            worksheet.write(row, 0, criterion, cell_format)
        ...
        if criteriaMatrix and len(criteriaMatrix) > 0:
            ...
            for i, crit1 in enumerate(criteria):
                worksheet.write(row, 0, crit1, cell_format)
                for j, crit2 in enumerate(criteria):
                    if i < len(criteriaMatrix) and j < len(criteriaMatrix[i]):
                        ...
                        if isinstance(value, str) and '/' in value:
                            ...
                            worksheet.write(row, j + 1, value, cell_center_format)
                        else:
        ...
        for i, alternative in enumerate(alternatives):
            worksheet.write(row, 0, alternative, cell_format)
            row += 1
        ...
        for i, criterion in enumerate(criteria):
            ...
            if alternativeMatrices and criterion in alternativeMatrices:
                ...
                if matrix:
                    for j, alt1 in enumerate(alternatives):
                        worksheet.write(row, 0, alt1, cell_format)
                        for k, alt2 in enumerate(alternatives):
                            if j < len(matrix) and k < len(matrix[j]):
                                ...
                                if isinstance(value, str) and '/' in value:
                                    ...
                                    worksheet.write(row, k + 1, value, cell_center_format)
                                else:
                                    # Đối với số thực, sử dụng định dạng số thập phân
                                    try:
                                ...
                                worksheet.write(row, k + 1, 1 if j == k else 0, decimal_format)
                        row += 1
                        ...
                        for j, alt1 in enumerate(alternatives):
                            ...
                            for k, alt2 in enumerate(alternatives):
                                if j == k:
                                    ...
                                    worksheet.write(row, k + 1, 1, decimal_format)
                                else:
            ...
            # Ghi trọng số của các phương án theo tiêu chí này
        ...
        # Ghi kết quả cuối cùng
        for i, alt in enumerate(sorted_alternatives):
            worksheet.write(row, 0, alt, cell_format)
            worksheet.write(row, 1, finalScores.get(alt, 0), decimal_format)
            worksheet.write(row, 2, i + 1, cell_center_format)
            row += 1
...
def export_pdf():
    ...
    try:
        ...
        from reportlab.platypus import Frame, PageTemplate, Image, Paragraph, Table, TableStyle, Spacer
        ...
        corporate_blue = colors.HexColor('#1a56db')  # Primary color
        corporate_light_blue = colors.HexColor('#dbeafe')  # Light background
        corporate_dark = colors.HexColor('#1e293b')  # Dark text
        corporate_green = colors.HexColor('#15803d')  # Success/highlight color
        corporate_light_green = colors.HexColor('#dcfce7')  # Light green background
        corporate_gray = colors.HexColor('#64748b')  # Subtle text
        corporate_light_gray = colors.HexColor('#f8fafc')  # Light background
        table_header_bg = corporate_blue
        table_header_text = colors.white
        table_alt_row = corporate_light_blue
        
        # Custom styles for a professional report
        styles.add(ParagraphStyle(
            name='ReportTitle', fontName='DejaVuSans', fontSize=16, 
            alignment=TA_CENTER, textColor=corporate_dark, 
            leading=22, spaceAfter=0.5*cm, spaceBefore=0
        ))
        styles.add(ParagraphStyle(
            name='CompanyHeader', fontName='DejaVuSans', fontSize=12, 
            alignment=TA_CENTER, textColor=corporate_blue, 
            leading=16, spaceAfter=0.3*cm, spaceBefore=0
        ))
        styles.add(ParagraphStyle(
            name='ReportSubtitle', fontName='DejaVuSans', fontSize=14, 
            alignment=TA_CENTER, textColor=corporate_dark, 
            leading=18, spaceAfter=0.8*cm, spaceBefore=0
        ))
        styles.add(ParagraphStyle(
            name='SectionHeader', fontName='DejaVuSans', fontSize=12, 
            alignment=TA_LEFT, textColor=corporate_blue, 
            leading=16, spaceAfter=0.3*cm, spaceBefore=0.5*cm
        ))
        styles.add(ParagraphStyle(
            name='TableHeader', fontName='DejaVuSans', fontSize=10, 
            alignment=TA_CENTER, textColor=table_header_text, 
            leading=14, spaceAfter=2, spaceBefore=2
        ))
        styles.add(ParagraphStyle(
            name='TableCell', fontName='DejaVuSans', fontSize=9, 
            alignment=TA_LEFT, leading=12, spaceAfter=1, 
            spaceBefore=1, wordWrap='CJK'
        ))
        styles.add(ParagraphStyle(
            name='TableCellCenter', fontName='DejaVuSans', fontSize=9, 
            alignment=TA_CENTER, leading=12, spaceAfter=1, 
            spaceBefore=1, wordWrap='CJK'
        ))
        styles.add(ParagraphStyle(
            name='TableCellRight', fontName='DejaVuSans', fontSize=9, 
            alignment=TA_RIGHT, leading=12, spaceAfter=1, 
            spaceBefore=1, wordWrap='CJK'
        ))
        styles.add(ParagraphStyle(
            name='ExecutiveSummary', fontName='DejaVuSans', fontSize=10, 
            alignment=TA_JUSTIFY, leading=14, 
            textColor=corporate_dark, 
            backColor=corporate_light_gray, 
            spaceAfter=0.5*cm, spaceBefore=0.3*cm, 
            leftIndent=0.5*cm, rightIndent=0.5*cm, 
            borderPadding=0.3*cm
        ))
        styles.add(ParagraphStyle(
            name='Conclusion', fontName='DejaVuSans', fontSize=10, 
            alignment=TA_JUSTIFY, leading=14, 
            textColor=corporate_green, 
            backColor=corporate_light_green, 
            spaceAfter=0.5*cm, spaceBefore=0.3*cm, 
            leftIndent=0.5*cm, rightIndent=0.5*cm, 
            borderPadding=0.3*cm
        ))
        styles.add(ParagraphStyle(
            name='Footer', fontName='DejaVuSans', fontSize=8, 
            alignment=TA_CENTER, textColor=corporate_gray, 
            leading=10, spaceBefore=0.5*cm
        ))
        styles.add(ParagraphStyle(
            name='PageNumber', fontName='DejaVuSans', fontSize=8, 
            alignment=TA_RIGHT, textColor=corporate_gray, 
            leading=10
        ))
        
        # Add a logo or header image (if available)
        # elements.append(Image("path/to/logo.png", width=5*cm, height=1.5*cm))
        # elements.append(Spacer(1, 0.5*cm))
        
        # Company header
        elements.append(Paragraph("<b>BÁO CÁO PHÂN TÍCH QUYẾT ĐỊNH</b>", styles['CompanyHeader']))
        
        # Report title
        elements.append(Paragraph("<b>PHÂN TÍCH PHƯƠNG PHÁP AHP</b>", styles['ReportTitle']))
        elements.append(Spacer(1, 0.2*cm))
        
        # Report subtitle with goal
        elements.append(Paragraph(f"<i>{goal}</i>", styles['ReportSubtitle']))
        elements.append(Spacer(1, 0.5*cm))
        
        # Add a horizontal line
        elements.append(Paragraph("<hr width=100%>", styles['TableCell']))
        elements.append(Spacer(1, 0.5*cm))
        
        # Executive summary
        summary_text = f"""
        <b>Tóm tắt báo cáo:</b> Báo cáo này trình bày kết quả phân tích quyết định sử dụng phương pháp 
        Quá trình phân tích thứ bậc (AHP - Analytic Hierarchy Process). Phân tích được thực hiện dựa trên 
        {len(criteria)} tiêu chí đánh giá và {len(alternatives)} phương án lựa chọn. 
        Mục tiêu của phân tích là "<i>{goal}</i>".
        """
        elements.append(Paragraph(summary_text, styles['ExecutiveSummary']))
        
        # Section: Criteria Weights
        elements.append(Paragraph("<b>1. TRỌNG SỐ CÁC TIÊU CHÍ</b>", styles['SectionHeader']))
        elements.append(Spacer(1, 0.2*cm))
        
        # Criteria weights description
        criteria_desc = """
        Bảng dưới đây trình bày trọng số của từng tiêu chí đánh giá, được tính toán 
        dựa trên phương pháp AHP. Trọng số thể hiện mức độ quan trọng tương đối của 
        mỗi tiêu chí trong việc đưa ra quyết định cuối cùng.
        """
        elements.append(Paragraph(criteria_desc, styles['TableCell']))
        elements.append(Spacer(1, 0.3*cm))
        
        # Table data for criteria weights
        criteria_data = [
            [Paragraph('<b>Tiêu chí</b>', styles['TableHeader']),
             Paragraph('<b>Trọng số</b>', styles['TableHeader']),
             Paragraph('<b>Tỷ lệ</b>', styles['TableHeader'])]
        ]
        
        # Sort criteria by weight for better presentation
        criteria_with_weights = [(criteria[i], criteriaWeights[i]) for i in range(len(criteria))]
        criteria_with_weights.sort(key=lambda x: x[1], reverse=True)
        
        for criterion, weight in criteria_with_weights:
            criteria_data.append([
                Paragraph(criterion, styles['TableCell']),
                Paragraph(f"{weight:.4f}", styles['TableCellRight']),
                Paragraph(f"{weight*100:.2f}%", styles['TableCellRight'])
            ])
        
        criteria_table = Table(criteria_data, colWidths=[10*cm, 3*cm, 3*cm], repeatRows=1)
        criteria_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 0), (-1, 0), 10),
            ('FONTSIZE', (0, 1), (-1, -1), 9),
            ('ROWBACKGROUNDS', (0, 1), (-1, -1), [corporate_light_gray, corporate_light_blue]),
            ('GRID', (0, 0), (-1, -1), 0.5, corporate_gray),
        ...
        # Add a pie chart visualization for criteria weights
        # (This would require additional libraries - placeholder for future enhancement)
        elements.append(Spacer(1, 0.8*cm))
        
        # Section: Final Results
        elements.append(Paragraph("<b>2. KẾT QUẢ PHÂN TÍCH CUỐI CÙNG</b>", styles['SectionHeader']))
        elements.append(Spacer(1, 0.2*cm))
        
        # Final results description
        final_desc = """
        Bảng dưới đây trình bày kết quả phân tích cuối cùng, xếp hạng các phương án theo 
        điểm số tổng hợp từ tất cả các tiêu chí. Điểm số được tính bằng cách nhân trọng số 
        của mỗi tiêu chí với điểm số của phương án theo tiêu chí đó, sau đó cộng lại.
        """
        elements.append(Paragraph(final_desc, styles['TableCell']))
        elements.append(Spacer(1, 0.3*cm))
        ...
        final_data = [
            [Paragraph('<b>Xếp hạng</b>', styles['TableHeader']),
             Paragraph('<b>Phương án</b>', styles['TableHeader']),
             Paragraph('<b>Điểm số</b>', styles['TableHeader']),
             Paragraph('<b>Tỷ lệ</b>', styles['TableHeader'])]
        ]
        ...
        for i, alt in enumerate(sorted_alternatives):
            ...
            row = [
                ...
                Paragraph(f"{score:.4f}", styles['TableCellRight']),
                Paragraph(f"{percentage:.2f}%", styles['TableCellRight'])
            ]
            final_data.append(row)
        
        final_table = Table(final_data, colWidths=[2*cm, 10*cm, 2.5*cm, 2.5*cm], repeatRows=1)
        
        # Best alternative row style
        best_row_color = corporate_light_green
        ...
        if len(criteria) > 1 and len(alternatives) > 1:
            elements.append(Paragraph("<b>3. PHÂN TÍCH CHI TIẾT</b>", styles['SectionHeader']))
            elements.append(Spacer(1, 0.2*cm))
            
            analysis_text = f"""
            Phân tích cho thấy tiêu chí <b>{criteria_with_weights[0][0]}</b> có trọng số cao nhất 
            ({criteria_with_weights[0][1]:.4f} hay {criteria_with_weights[0][1]*100:.2f}%), 
            đóng vai trò quan trọng nhất trong việc đưa ra quyết định.
            """
            elements.append(Paragraph(analysis_text, styles['TableCell']))
            elements.append(Spacer(1, 0.3*cm))
        ...
        if sorted_alternatives:
            ...
            conclusion_text = f"""
            <b>KẾT LUẬN:</b> Dựa trên phân tích AHP, phương án <b>"{best_alt}"</b> là lựa chọn tối ưu 
            với điểm số <b>{best_score:.4f}</b> (<b>{percentage:.2f}%</b> tổng điểm).
            """
            
            if len(sorted_alternatives) > 1:
                second_alt = sorted_alternatives[1]
                second_score = finalScores.get(second_alt, 0)
                diff_percent = ((best_score - second_score) / second_score) * 100 if second_score > 0 else 0
                
                conclusion_text += f"""
                Phương án này vượt trội hơn phương án xếp hạng thứ hai (<b>"{second_alt}"</b>) 
                khoảng <b>{diff_percent:.2f}%</b>.
                """
            
            # Add recommendation
            conclusion_text += f"""
            <b>KHUYẾN NGHỊ:</b> Nên triển khai phương án <b>"{best_alt}"</b> dựa trên các tiêu chí 
            đã được phân tích và đánh giá.
            """
        ...
        signature_data = [
            [Paragraph('<b>Người phê duyệt:</b>', styles['TableCell']), 
             Paragraph('<b>Người thực hiện:</b>', styles['TableCell'])],
            ...
            [Paragraph('', styles['TableCell']), 
             Paragraph('', styles['TableCell'])],
            [Paragraph('<i>(Ký, ghi rõ họ tên)</i>', styles['TableCellCenter']), 
             Paragraph('<i>(Ký, ghi rõ họ tên)</i>', styles['TableCellCenter'])]
        ]
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.setFillColor(corporate_gray)
            
            # Add a line at the bottom
            canvas.setStrokeColor(corporate_blue)
            ...
            canvas.drawRightString(doc.width + doc.leftMargin - 0.5*cm, 1.5*cm, f"Trang {page_num}")

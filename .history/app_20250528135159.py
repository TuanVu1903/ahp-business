...
def export_excel():
    ...
    try:
        ...
        for j, criterion in enumerate(criteria):
            worksheet.write(row, j + 1, criterion, header_format)
        row += 1
        
        # Ghi ma trận so sánh tiêu chí
        if criteriaMatrix and len(criteriaMatrix) == len(criteria):
            # Nếu có ma trận so sánh tiêu chí từ dữ liệu gửi lên
            for i, criterion in enumerate(criteria):
                worksheet.write(row, 0, criterion)
                for j in range(len(criteria)):
                    if i < len(criteriaMatrix) and j < len(criteriaMatrix[i]):
                        worksheet.write(row, j + 1, criteriaMatrix[i][j], decimal_format)
                    else:
                        # Giá trị mặc định nếu không có dữ liệu
                        worksheet.write(row, j + 1, 1 if i == j else 0, decimal_format)
                row += 1
        else:
            # Nếu không có ma trận, tạo ma trận từ trọng số
            for i, criterion1 in enumerate(criteria):
                worksheet.write(row, 0, criterion1)
                for j, criterion2 in enumerate(criteria):
                    if i == j:
                        # Đường chéo chính là 1
                        worksheet.write(row, j + 1, 1, decimal_format)
                    else:
                        # Ước lượng tỷ lệ dựa trên trọng số
                        value = criteriaWeights[i] / criteriaWeights[j] if criteriaWeights[j] != 0 else 1
            ...
            for j, alt1 in enumerate(alternatives):
                ...
                for k, alt2 in enumerate(alternatives):
                    if j == k:
                        ...
                        # Lấy giá trị so sánh từ dữ liệu điền vào hoặc mặc định là 1
                        # Để đơn giản hóa và đảm bảo khả năng import lại, lưu giá trị dưới dạng số
                        if alternativeScores.get(alt1) and alternativeScores[alt1].get(criterion):
...
def import_excel():
    ...
    try:
        ...
        ahp_data = {
            ...
            "criteria": [],
            "criteria_weights": [],
            "criteria_matrix": [],  # Ma trận so sánh tiêu chí
            "alternatives": [],
        ...
        try:
            ...
            # 2.1 Đọc ma trận so sánh tiêu chí
            criteria_matrix_start = None
            for i, row in data_df.iterrows():
                if row.iloc[0] == 'MA TRẬN SO SÁNH CÁC TIÊU CHÍ':
                    criteria_matrix_start = i
                    break
                    
            if criteria_matrix_start is not None:
                # Bỏ qua dòng tiêu đề
                matrix_header_row = criteria_matrix_start + 1
                matrix_start_row = matrix_header_row + 1
                
                # Tạo ma trận tiêu chí trống
                criteria_matrix = []
                
                # Đọc ma trận từ file Excel
                for i in range(len(ahp_data["criteria"])):
                    row_data = []
                    for j in range(len(ahp_data["criteria"])):
                        # Kiểm tra index hợp lệ
                        if matrix_start_row + i < len(data_df) and j + 1 < len(data_df.columns):
                            value = data_df.iloc[matrix_start_row + i, j + 1]
                            if not pd.isna(value):
                                try:
                                    row_data.append(float(value))
                                except (ValueError, TypeError):
                                    row_data.append(1 if i == j else 0)
                            else:
                                row_data.append(1 if i == j else 0)
                        else:
                            row_data.append(1 if i == j else 0)
                    criteria_matrix.append(row_data)
            ...
            for idx, (start_idx, criterion) in enumerate(matrix_start_indices):
                ...
                for i in range(n):
                    for j in range(n):
                        if i < len(matrix_data) and j + 1 < len(matrix_data.columns):
                            ...
                            if not pd.isna(value):
                                ...
                                try:
                                    ...
                                    comparison_matrix[i][j] = float_val
                                except (ValueError, TypeError):
                                    # Nếu không phải số, giữ nguyên giá trị mặc định (1)
                                    pass
                        
                # Đảm bảo ma trận đối xứng nghịch đảo
                for i in range(n):
                    for j in range(i+1, n):
                        # Chỉ cần đảm bảo tính chất đối xứng ở tam giác trên
                        if i != j:

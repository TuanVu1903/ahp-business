...
def export_pdf():
    ...
    try:
        ...
        if sorted_alternatives:
            ...
            # Split conclusion into multiple paragraphs
            conclusion_main = f"""
            <para>
            Dựa trên kết quả phân tích AHP với {len(criteria)} tiêu chí và {len(alternatives)} phương án, 
            phương án được đánh giá là phù hợp nhất là:
            </para>
            """
            elements.append(Paragraph(conclusion_main, styles['CorporateBodyText']))
            
            # Best alternative as highlighted text
            best_alt_text = f"<para><b>{best_alt}</b></para>"
            elements.append(Paragraph(best_alt_text, styles['CorporateHighlight']))
            
            # Score details
            score_text = f"""
            <para>
            với điểm số <b>{best_score:.4f}</b> (chiếm <b>{percentage:.2f}%</b> tổng điểm).
            </para>
            """
            elements.append(Paragraph(score_text, styles['CorporateBodyText']))

...
def export_pdf():
    ...
    try:
        ...
        styles.add(ParagraphStyle(
            name='CustomSummary', fontName='DejaVuSans', fontSize=11, alignment=TA_JUSTIFY, leading=15, textColor=colors.HexColor('#1e293b'), spaceAfter=12, spaceBefore=0
        ))
        styles.add(ParagraphStyle(
            name='CustomNote', fontName='DejaVuSans', fontSize=10, alignment=TA_LEFT, leading=14, textColor=colors.HexColor('#64748b'), spaceAfter=6, spaceBefore=0, leftIndent=8
        ))

        # Header with logo placeholder and company info
        header_table_data = [
            [Image('static/images/logo.png', width=60, height=60) if os.path.exists('static/images/logo.png') else '',
             Paragraph("<b>BÁO CÁO PHÂN TÍCH RA QUYẾT ĐỊNH<br/>PHƯƠNG PHÁP AHP</b>", styles['CustomHeader'])]
        ]
        header_table = Table(header_table_data, colWidths=[70, 450])
        ...
        doc_info_table = Table(doc_info_data, colWidths=[100, 150])
        ...
        # Executive Summary
        elements.append(Paragraph("TÓM TẮT PHÂN TÍCH", styles['CustomSection']))
        summary_text = f"""
        Báo cáo này trình bày kết quả phân tích quyết định đa tiêu chí sử dụng phương pháp AHP (Analytic Hierarchy Process) 
        cho mục tiêu: <b>{goal}</b>. Phân tích được thực hiện dựa trên <b>{len(criteria)} tiêu chí</b> và <b>{len(alternatives)} phương án</b> 
        thay thế. Kết quả phân tích sẽ giúp đưa ra quyết định khách quan và tối ưu nhất dựa trên các tiêu chí đã được xác định.
        """
        elements.append(Paragraph(summary_text, styles['CustomSummary']))
        elements.append(Spacer(1, 0.1*inch))

        # Section: Criteria Weights
        elements.append(Paragraph("TRỌNG SỐ CÁC TIÊU CHÍ", styles['CustomSection']))
        # Add note about criteria
        elements.append(Paragraph("* Trọng số thể hiện mức độ quan trọng tương đối của các tiêu chí trong việc ra quyết định", 
                                styles['CustomNote']))
        ...
        elements.append(Paragraph("* Kết quả được sắp xếp theo thứ tự điểm số từ cao đến thấp", styles['CustomNote']))
        ...
        if sorted_alternatives:
            ...
            analysis_text = f"""<b>PHÂN TÍCH CHI TIẾT:</b><br/><br/>
            ...
            (chiếm <b>{percentage:.2f}%</b> tổng điểm).<br/><br/>"""
            ...
            if len(sorted_alternatives) > 1:
                ...
                analysis_text += f"""So với phương án xếp hạng thứ hai (<b>{second_alt}</b>), 
                phương án được chọn có điểm số cao hơn <b>{diff_percent:.2f}%</b>. 
                Sự chênh lệch này cho thấy tính vượt trội của phương án được chọn."""
            
            elements.append(Paragraph(analysis_text, styles['CustomConclusion']))

        # Recommendations section
        elements.append(Spacer(1, 0.1*inch))
        elements.append(Paragraph("KHUYẾN NGHỊ", styles['CustomSection']))
        recommendations = f"""
        1. Nên triển khai phương án <b>{sorted_alternatives[0]}</b> vì đây là phương án tối ưu nhất dựa trên các tiêu chí đã xác định.
        
        2. Cần theo dõi và đánh giá lại các tiêu chí định kỳ để đảm bảo tính phù hợp của quyết định.
        
        3. Lưu ý các rủi ro và có kế hoạch dự phòng trong quá trình thực hiện.
        """
        elements.append(Paragraph(recommendations, styles['CustomSummary']))

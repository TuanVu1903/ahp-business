...
# Danh sách các từ ngữ cấm
PROHIBITED_WORDS = [
    "trốn thuế", "biển thủ", "gian lận", "rửa tiền", "tham nhũng", 
    "lừa đảo", "bất hợp pháp", "phi pháp", "phạm pháp", "phạm tội",
    "gian dối", "giả mạo", "làm giả", "trái phép", "vi phạm"
]

def check_prohibited_words(text):
    """
    Kiểm tra xem văn bản có chứa từ ngữ cấm hay không.
    
    Args:
        text: Văn bản cần kiểm tra
        
    Returns:
        tuple: (bool, list) - (có từ cấm hay không, danh sách các từ cấm tìm thấy)
    """
    if not text:
        return False, []
...
def generate_criteria_matrix():
    ...
    try:
        ...
        error_response = check_goal_for_prohibited_words(goal)
        if error_response:
...
def check_goal_for_prohibited_words(goal):
    has_prohibited_words, found_words = check_prohibited_words(goal)
    if has_prohibited_words:
        return jsonify({"error": f"Mục tiêu chứa từ ngữ không phù hợp: {', '.join(found_words)}. Vui lòng chỉnh sửa mục tiêu của bạn."}), 400
    return None

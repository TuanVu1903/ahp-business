...
def export_pdf():
    ...
    try:
        ...
        # Custom styles for enterprise look
        styles.add(ParagraphStyle(
            name='CorporateCoverHeader',  # Changed from CoverHeader
            fontName='DejaVuSans',
            fontSize=24,
            alignment=TA_CENTER,
        ...
        styles.add(ParagraphStyle(
            name='CorporateCoverSubHeader',  # Changed from CoverSubHeader
            fontName='DejaVuSans',
            fontSize=16,
            alignment=TA_CENTER,
        ...
        styles.add(ParagraphStyle(
            name='CorporateSectionHeader',  # Changed from SectionHeader
            fontName='DejaVuSans',
            fontSize=14,
            alignment=TA_LEFT,
            textColor=PRIMARY_COLOR,
        ...
        styles.add(ParagraphStyle(
            name='CorporateSubSection',  # Changed from SubSection
            fontName='DejaVuSans',
            fontSize=12,
            alignment=TA_LEFT,
            textColor=SECONDARY_COLOR,
        ...
        styles.add(ParagraphStyle(
            name='CorporateBodyText',  # Changed from BodyText
            fontName='DejaVuSans',
            fontSize=11,
        ...
        styles.add(ParagraphStyle(
            name='CorporateTableHeader',  # Changed from TableHeader
            fontName='DejaVuSans',
            fontSize=10,
            alignment=TA_CENTER,
        ...
        styles.add(ParagraphStyle(
            name='CorporateTableCell',  # Changed from TableCell
            fontName='DejaVuSans',
            fontSize=10,
            alignment=TA_LEFT,
            leading=14,
        ...
        styles.add(ParagraphStyle(
            name='CorporateNote',  # Changed from Note
            fontName='DejaVuSans',
            fontSize=9,
            alignment=TA_LEFT,
        ...
        if os.path.exists('static/images/logo.png'):
            img = Image('static/images/logo.png', width=100, height=100)
            img.hAlign = 'CENTER'
            elements.append(img)
            elements.append(Spacer(1, 30))

        elements.append(Paragraph("BÁO CÁO PHÂN TÍCH", styles['CoverHeader']))
        elements.append(Paragraph("PHƯƠNG PHÁP PHÂN TÍCH THỨU BẬC (AHP)", styles['CoverSubHeader']))
        
        elements.append(Spacer(1, 30))
        elements.append(Paragraph(f"<b>{goal}</b>", styles['CoverSubHeader']))
        
        # Document info on cover
        elements.append(Spacer(1, 60))
        doc_info_data = [
        ...
        # Table of Contents
        elements.append(Paragraph("MỤC LỤC", styles['SectionHeader']))
        ...
        toc_table = Table(
            ...
            style=TableStyle([
                ...
                ('LINEBELOW', (0, -1), (-1, -1), 1, SUBTEXT_COLOR),
            ])
        )
        ...
        # 1. Overview section
        elements.append(Paragraph("1. TỔNG QUAN", styles['SectionHeader']))
        overview_text = f"""
        ...
        elements.append(Paragraph("2. PHƯƠNG PHÁP LUẬN", styles['SectionHeader']))
        method_text = """
        ...
        """
        elements.append(Paragraph(method_text, styles['BodyText']))

        # 3. Criteria Analysis
        elements.append(Paragraph("3. PHÂN TÍCH TIÊU CHÍ", styles['SectionHeader']))
        elements.append(Paragraph("3.1 Trọng số các tiêu chí", styles['SubSection']))
        ...
        # Criteria weights table with improved styling
        criteria_data = [
            [Paragraph('Tiêu chí', styles['TableHeader']),
             Paragraph('Trọng số', styles['TableHeader']),
             Paragraph('Tỷ trọng', styles['TableHeader'])]
        ]
        
        for i, criterion in enumerate(criteria):
            criteria_data.append([
                Paragraph(criterion, styles['TableCell']),
                Paragraph(f"{criteriaWeights[i]:.4f}", styles['TableCell']),
                Paragraph(f"{criteriaWeights[i]*100:.2f}%", styles['TableCell'])
            ])
        ...
        elements.append(Paragraph("4. ĐÁNH GIÁ PHƯƠNG ÁN", styles['SectionHeader']))
        elements.append(Paragraph("4.1 Kết quả đánh giá tổng hợp", styles['SubSection']))
        ...
        for i, alt in enumerate(sorted_alternatives):
            ...
            final_data.append([
                ...
                Paragraph(f"{score:.4f}", styles['TableCell']),
                Paragraph(f"{percentage:.2f}%", styles['TableCell'])
            ])
        ...
        elements.append(Paragraph("5. KẾT LUẬN VÀ KHUYẾN NGHỊ", styles['SectionHeader']))
        
        # Conclusions
        elements.append(Paragraph("5.1 Kết luận", styles['SubSection']))
        ...
        # Recommendations
        elements.append(Paragraph("5.2 Khuyến nghị", styles['SubSection']))
        recommendations = f"""
        <b>1. Triển khai thực hiện:</b>
        <br/>
        • Ưu tiên triển khai phương án <b>{sorted_alternatives[0]}</b>
        <br/>
